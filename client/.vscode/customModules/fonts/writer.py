...
class Writer():
    ...
    def __init__(self, device, font, verbose=True, darkMode = False):
    ...
    def _get_char(self, char, recurse):
        if not recurse:  # Handle tabs
            if char == '\n':
                ...
                while nspaces:
                    nspaces -= 1
                    self._printchar(' ', invert=False, recurse=True)
...
class ThreeColorWriter(Writer):

    def writeText(self, text, invert = False):
        self._newline()
        self._printline(text, invert)

    def writeTextBlack(self, text):
        self.bgcolor = 0xff
        self.fgcolor = 0
        self.writeText(text, True)

    def writeTextRed(self, text):
        self.bgcolor = 0
        self.fgcolor = 0xff
